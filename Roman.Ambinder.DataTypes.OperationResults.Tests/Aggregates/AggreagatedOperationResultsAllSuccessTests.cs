...
{
    [TestClass]
    public class AggreagatedOperationResultsAllSuccessTests
    {
        [TestMethod]
        public void AllSucessfulOpRes_Evaluate_SuccessfulOpRes()
        {
            ...
            //Act 
            var actualOpResAggreagte = evaluator.Evaluate(operationResults);
        ...
        [TestMethod]
        public void AllFailedOpResWithErrorMessage_Evaluate_MatchingFailedOpRes()
        {
            //Arrange
            var evaluator = GetEvaluator();
            var operationResults = new[] { new OperationResult(false, "Error1"), new OperationResult(false, "Error2") };
            var expectedErrorMessage = GetExpectedErrorMessages(operationResults);
            const bool expectedSuccessValue = false;
